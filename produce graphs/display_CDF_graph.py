...
if __name__ == '__main__':
    ...
    for i, col_name in enumerate(data.columns):
        ...
        ax.plot(x, y, label=col_name, marker=markers[i % len(markers)], markevery=400)
        plt.xlabel('Congestion (Utilization)')
        plt.ylabel('CDF \n (fraction of edges)')
    ...
    plt.legend()
    plt.savefig('graphs/' + sys.argv[1].split('/')[2] + "_congestion_distribution.pdf")
    plt.show()
